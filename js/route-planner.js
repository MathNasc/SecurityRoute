/* ════════════════════════════════════════════════════
   ...
   Manages the "Rota Segura" panel:
     1. Waypoint picking (click on map or reverse-geocode)
     2. Profile selection (foot / driving / cycling)
     3. OSRM route fetch
     4. Risk scoring via RiskEngine
     5. Card rendering + route layer management
   ════════════════════════════════════════════════════ */
...
const RoutePlanner = (() => {
  ...
  // ── Show/hide the map picking hint banner ───────────
  ...
  function _drawRoutes(routes, selectedIdx) {
    ...
    // Draw unselected routes first (lower z-index)
    routes.forEach((rt, i) => {
      if (i === selectedIdx) return;
      const line = L.polyline(rt.coords, _lineStyle(rt, false)).addTo(_map);
    ...
    const selLine = L.polyline(sel.coords, _lineStyle(sel, true)).addTo(_map);
    selLine.on('click', () => _selectRoute(selectedIdx));
    _routeLayers.push(selLine);

    // Hotspot circles on the selected route only
  ...
  async function _calculate() {
    ...
    try {
      ...
      // 4. Render map layers and cards
      _drawRoutes(scored, 0);
      _renderCards(scored);

      // 5. Fit map to all routes
      const allCoords = scored.flatMap(r => r.coords);
      if (allCoords.length) {
        _map.fitBounds(L.latLngBounds(allCoords).pad(0.14), { animate: true, duration: 0.6 });
      }

      Toast.success(
        `${scored.length} rota${scored.length > 1 ? 's' : ''} calculada${scored.length > 1 ? 's' : ''}`,
        `Risco mais baixo: ${scored[0].safetyLevel} (${scored[0].riskScore}/100)`
      );
  ...
  // ── Public: handle a map click ──────────────────────
  // Returns true if the click was consumed (pick mode active).
  function onMapClick(lat, lng) {
    if (!_pickMode) return false;
    const mode = _pickMode;
    _setPickMode(null);

    // Reverse-geocode asynchronously; set point immediately with coords as label
    ...
    // Update label once geocode resolves
  ...
  function init() {
    ...
    document.getElementById('rpSwap')?.addEventListener('click', () => {
      ...
      else { _oLat = null; const i = document.getElementById('rpOriginInp'); if (i) i.value = ''; if (_originMarker) { _originMarker.remove(); _originMarker = null; } }
      if (oL) _setDest(oL, oN, oLb);
      else { _dLat = null; const i = document.getElementById('rpDestInp');   if (i) i.value = ''; if (_destMarker)   { _destMarker.remove();   _destMarker   = null; } }
      _checkReady();
    });
    ...
    // Sidebar tab switch — switch to route panel programmatically
